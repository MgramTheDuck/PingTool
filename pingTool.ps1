...
while ($true) {
    ...
    $timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
      if ($result) {
        # Handle different property names in different PowerShell versions
        ...
        elseif ($null -ne $result.ResponseTime) {
            ...
            # Even older versions might use this
            $time = $result.ResponseTimeToLive
        }
        ...
        $msg = "$timestamp - Reply from $address : Latency=${time}ms, Buffer=$bufferSize bytes, Min=${minTime}ms, Max=${maxTime}ms, Avg=${avgTime}ms"
    } else {
        $failCount++
